...
apt install -y nfs-kernel-server ufw # устанавливаем межсетевой экран и nfs сервер
...
systemctl restart nfs-kernel-server # перезагружаем службу nfs, чтобы папка стала активна
